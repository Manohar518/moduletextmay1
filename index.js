...
let heading = document.getElementById("welocome-heading");
...
function showItems(foodItems) {
  ...
  heading.innerHTML = "Welcome to Our Menu !";
...
let isOrderPrep=(status) => {
    ...
    orderPaid.innerText=`${status.paid ?`Order Paid :SuccessFully !`:`Pending..`}`
...
let isOrderPaid=(status) => {
  ...
  foodItemContainer.innerHTML="";
  foodItemContainer.append(orderPaid);
  heading.innerText='ThankYou for Coming Our Restaurant :) !'
...
function thankyouFnc() {
  alert('Thankyou For Enjoying Delicious Dishes :) ');
//   console.log("Thankyou for Coming Our Restaurant");
}